...
*     .. External Subroutines ..
     ...
     $                   INFOG2L, PCELGET, PCMAX1,
     $                   PSCSUM1, SGEBR2D, SGEBS2D
*     ..
*     .. External Functions ..
      INTEGER            INDXG2L, INDXG2P, INDXL2G, NUMROC
      REAL               PSLAMCH
      EXTERNAL           INDXG2L, INDXG2P, INDXL2G, NUMROC, PSLAMCH
*     ..
*     .. Intrinsic Functions ..
...
*        ... QUIT
         GO TO 130
      END IF
      CALL PSCSUM1( N, EST, X, IX, JX, DESCX, 1 )
...
*
   40 CONTINUE
      CALL PCMAX1( N, XMAX, J, X, IX, JX, DESCX, 1 )
      IF( DESCX( M_ ).EQ.1 .AND. N.EQ.1 ) THEN
         IF( MYROW.EQ.IVXROW ) THEN
            WORK( 1 ) = XMAX
            WORK( 2 ) = CMPLX( REAL( J ) )
            CALL CGEBS2D( ICTXT, 'Columnwise', ' ', 2, 1, WORK, 2 )
         ELSE
            CALL CGEBR2D( ICTXT, 'Columnwise', ' ', 2, 1, WORK, 2,
     $                    IVXROW, MYCOL )
            XMAX = WORK( 1 )
            J = NINT( REAL( WORK( 2 ) ) )
         END IF
...
*
   70 CONTINUE
      CALL CCOPY( NP, X( IOFFVX ), 1, V( IOFFVX ), 1 )
      ESTOLD = EST
      CALL PSCSUM1( N, EST, V, IV, JV, DESCV, 1 )
...
*
   90 CONTINUE
     ...
     $                    IVXROW, MYCOL )
         ...
         END IF
      END IF
      CALL PCELGET( 'Columnwise', ' ', JLMAX, X, JLAST, JX, DESCX )
      IF( ( REAL( JLMAX ).NE.ABS( REAL( XMAX ) ) ).AND.
     $    ( ITER.LT.ITMAX                        ) ) THEN
         ITER = ITER + 1
         GO TO 50
      END IF
  ...
  100 CONTINUE
     ...
     $                DESCX( RSRC_ ), NPROW )-IX+1
         ...
         ELSE
            ALTSGN = ONE
         END IF
         X( I ) = CMPLX( ALTSGN*( ONE+REAL( K-1 ) / REAL( N-1 ) ) )
  110 CONTINUE
      KASE = 1
      JUMP = 5
      RETURN
...
*
  120 CONTINUE
      CALL PSCSUM1( N, TEMP, X, IX, JX, DESCX, 1 )
      IF( DESCX( M_ ).EQ.1 .AND. N.EQ.1 ) THEN
         IF( MYROW.EQ.IVXROW ) THEN
