...
*  =======
*
*  PDPTDRIVER is a test program for the
*  ScaLAPACK Band Cholesky routines corresponding to the options
*  indicated by DPT.  This test driver performs an
...
*     .. External Subroutines ..
     ...
     $                   BLACS_PINFO, DESCINIT, IGSUM2D, PDBMATGEN,
     $                   PDCHEKPAD, PDFILLPAD, PDMATGEN, PDPTINFO,
     $                   PDPTLASCHK, PDPTTRF, PDPTTRS, SLBOOT,
     $                   SLCOMBINE, SLTIMER
*     ..
*     .. External Functions ..
      INTEGER            NUMROC
...
*
      ...
      IBSEED = 200
*
      CALL PDPTINFO( OUTFILE, NOUT, UPLO, NMAT, NVAL, NTESTS, NBW,
...
*
     ...
     $               WRITE( NOUT, FMT = 9997 ) 'MEMORY'
               ...
               END IF
*
               CALL PDBMATGEN( ICTXT, UPLO, 'T', BW, BW, N, (2), NB,
     $                         MEM( IPA ), NB+10, 0, 0, IASEED, MYROW,
     $                         MYCOL, NPROW, NPCOL )
               CALL PDFILLPAD( ICTXT, NQ, NP, MEM( IPA-IPREPAD ),
...
*
     ...
     $                            DESCA2D, MEM( IP_DRIVER_W ) )
                  CALL PDCHEKPAD( ICTXT, 'PDLANGE', NQ, NP,
...
*
               CALL SLTIMER( 1 )
*
               CALL PDPTTRF( N, MEM( IPA+INT_TEMP ),
...
*
               IF( INFO.NE.0 ) THEN
                  IF( IAM.EQ.0 ) THEN
                    WRITE( NOUT, FMT = * ) 'PDPTTRF INFO=', INFO
                  ENDIF
                  KFAIL = KFAIL + 1
                  GO TO 30
               END IF
...
*
                  CALL PDCHEKPAD( ICTXT, 'PDPTTRF', NQ,
...
*
                     CALL PDPTTRS( N, NRHS, MEM( IPA+INT_TEMP ),
...
*
                     IF( INFO.NE.0 ) THEN
                       IF( IAM.EQ.0 )
     $  WRITE( NOUT, FMT = * ) 'PDPTTRS INFO=', INFO
                       KFAIL = KFAIL + 1
                       PASSED = 'FAILED'
                       GO TO 20
                     END IF
...
*
                        CALL PDCHEKPAD( ICTXT, 'PDPTTRS-work',
     $                                  WORKSIZ, 1,
     $                                  MEM( IP_DRIVER_W-IPREPAD ),
     $                                  WORKSIZ, IPREPAD,
     $                                  IPOSTPAD, PADVAL )
...
*
     ...
     $                       (2), NB, 0, 0,
     $                       ICTXT, (2), IERR( 1 ) )
                        CALL PDPTLASCHK( 'S', UPLO, N, BW, BW, NRHS,
     $                              MEM( IPB ), 1, 1, DESCB2D,
