...
*  =======
*
*  PSDBDRIVER is a test program for the
*  ScaLAPACK Band Cholesky routines corresponding to the options
*  indicated by SDB.  This test driver performs an
*  A = L*U factorization
...
*     .. External Subroutines ..
     ...
     $                   PSCHEKPAD, PSDBINFO, PSDBLASCHK, PSDBTRF,
     $                   PSDBTRS, PSFILLPAD, PSMATGEN, SLBOOT,
     $                   SLCOMBINE, SLTIMER
*     ..
*     .. External Functions ..
      INTEGER            NUMROC
...
*
      ...
      IBSEED = 200
*
      CALL PSDBINFO( OUTFILE, NOUT, TRANS, NMAT, NVAL, NTESTS, NBW,
...
*
     ...
     $               WRITE( NOUT, FMT = 9997 ) 'MEMORY'
               ...
               END IF
*
               CALL PSBMATGEN( ICTXT, 'G', 'D', BWL, BWU, N,
...
*
               CALL SLTIMER( 1 )
*
               CALL PSDBTRF( N, BWL, BWU, MEM( IPA ), 1, DESCA,
...
*
               IF( INFO.NE.0 ) THEN
                  IF( IAM.EQ.0 ) THEN
                    WRITE( NOUT, FMT = * ) 'PSDBTRF INFO=', INFO
                  ENDIF
                  KFAIL = KFAIL + 1
                  GO TO 30
               END IF
...
*
                  CALL PSCHEKPAD( ICTXT, 'PSDBTRF', NP,
...
*
                     CALL PSDBTRS( TRANS, N, BWL, BWU, NRHS, MEM( IPA ),
...
*
                     IF( INFO.NE.0 ) THEN
                       IF( IAM.EQ.0 )
     $  WRITE( NOUT, FMT = * ) 'PSDBTRS INFO=', INFO
                       KFAIL = KFAIL + 1
                       PASSED = 'FAILED'
                       GO TO 20
                     END IF
...
*
                        CALL PSCHEKPAD( ICTXT, 'PSDBTRS-work',
     $                                  WORKSIZ, 1,
     $                                  MEM( IP_DRIVER_W-IPREPAD ),
     $                                  WORKSIZ, IPREPAD,
     $                                  IPOSTPAD, PADVAL )
