...
*  =======
*
*  PCDTDRIVER is a test program for the
*  ScaLAPACK Band Cholesky routines corresponding to the options
*  indicated by CDT.  This test driver performs an
*  A = L*U factorization
...
*     .. External Subroutines ..
     ...
     $                   PCCHEKPAD, PCDTINFO, PCDTLASCHK, PCDTTRF,
     $                   PCDTTRS, PCFILLPAD, PCMATGEN, SLBOOT,
     $                   SLCOMBINE, SLTIMER
*     ..
*     .. External Functions ..
      INTEGER            NUMROC
...
*
      ...
      IBSEED = 200
*
      CALL PCDTINFO( OUTFILE, NOUT, TRANS, NMAT, NVAL, NTESTS, NBW,
...
*
     ...
     $               WRITE( NOUT, FMT = 9997 ) 'MEMORY'
               ...
               END IF
*
               CALL PCBMATGEN( ICTXT, 'T', 'D', BWL, BWU, N, (3), NB,
...
*
               CALL SLTIMER( 1 )
*
               CALL PCDTTRF( N, MEM( IPA+2*( NB+10 ) ),
...
*
               IF( INFO.NE.0 ) THEN
                  IF( IAM.EQ.0 ) THEN
                    WRITE( NOUT, FMT = * ) 'PCDTTRF INFO=', INFO
                  ENDIF
                  KFAIL = KFAIL + 1
                  GO TO 30
               END IF
...
*
                  CALL PCCHEKPAD( ICTXT, 'PCDTTRF', NQ,
...
*
                     IF( INFO.NE.0 ) THEN
                       IF( IAM.EQ.0 )
     $  WRITE( NOUT, FMT = * ) 'PCDTTRS INFO=', INFO
                       KFAIL = KFAIL + 1
                       PASSED = 'FAILED'
                       GO TO 20
                     END IF
...
*
                        CALL PCCHEKPAD( ICTXT, 'PCDTTRS-work',
     $                                  WORKSIZ, 1,
     $                                  MEM( IP_DRIVER_W-IPREPAD ),
     $                                  WORKSIZ, IPREPAD,
     $                                  IPOSTPAD, PADVAL )
