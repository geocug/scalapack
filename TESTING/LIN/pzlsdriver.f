...
*  =======
*
*  PZLSDRIVER is the main test program for the COMPLEX*16
...
*     .. Local Arrays ..
     ...
     $                   PVAL( NTESTS ), QVAL( NTESTS )
      DOUBLE PRECISION   CTIME( 1 ), RESULT( 2 ), WTIME( 1 )
...
*     .. External Subroutines ..
     ...
     $                   PZFILLPAD, PZGELS, PZGEMM, PZLACPY,
     $                   PZLSINFO, PZMATGEN, PDZNRM2,
     $                   PZDSCAL, PZQRT13, PZQRT16, SLBOOT,
     $                   SLCOMBINE, SLTIMER
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            ICEIL, ILCM, NUMROC
      DOUBLE PRECISION   PZLANGE, PZQRT14, PZQRT17
      EXTERNAL           ICEIL, ILCM, LSAME, NUMROC, PZLANGE,
     $                   PZQRT14, PZQRT17
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
...
*
      IASEED = 100
      IBSEED = 200
      CALL PZLSINFO( OUTFILE, NOUT, NMAT, MVAL, NTESTS, NVAL,
...
*
                  CALL PZQRT13( ISCALE, M, N, MEM( IPA ), 1, 1,
     $                          DESCA, ANORM, IASEED, MEM( IPW ) )
*
                  IF( CHECK ) THEN
                     CALL PZCHEKPAD( ICTXT, 'PZQRT13', MP, NQ,
     $                               MEM( IPA-IPREPAD ), DESCA( LLD_ ),
     $                               IPREPAD, IPOSTPAD, PADVAL )
                     CALL PZCHEKPAD( ICTXT, 'PZQRT13',
     $                               WORKSIZ-IPOSTPAD, 1,
     $                               MEM( IPW-IPREPAD ),
     $                               WORKSIZ-IPOSTPAD, IPREPAD,
     $                               IPOSTPAD, PADVAL )
...
*
                           IF( CHECK ) THEN
                              CALL PZFILLPAD( ICTXT, MNP, NRHSQ,
     $                                        MEM( IPX-IPREPAD ),
     $                                        DESCX( LLD_ ), IPREPAD,
     $                                        IPOSTPAD, PADVAL )
                              IF( TPSD ) THEN
                                 CALL PZFILLPAD( ICTXT, MP, NRHSQ,
     $                                           MEM( IPW-IPREPAD ),
     $                                           DESCW( LLD_ ), IPREPAD,
     $                                           IPOSTPAD, PADVAL )
                              ELSE
                                 CALL PZFILLPAD( ICTXT, NP, NRHSQ,
     $                                           MEM( IPW-IPREPAD ),
     $                                           DESCW( LLD_ ), IPREPAD,
     $                                           IPOSTPAD, PADVAL )
                              END IF
...
*
                           DO 10 JJ = 1, NRHS
                              CALL PDZNRM2( NCOLS, BNORM, MEM( IPW ),
     $                                      1, JJ, DESCW, 1 )
                              IF( BNORM.GT.RZERO )
   ...
   10                      CONTINUE
*
                           CALL PZGEMM( TRANS, 'N', NROWS, NRHS, NCOLS,
...
*
                              CALL PZCHEKPAD( ICTXT, 'Generation', MP,
     $                                        NQ, MEM( IPA-IPREPAD ),
     $                                        DESCA( LLD_ ), IPREPAD,
     $                                        IPOSTPAD, PADVAL )
                              CALL PZCHEKPAD( ICTXT, 'Generation', MNP,
     $                                        NRHSQ, MEM( IPX-IPREPAD ),
     $                                        DESCX( LLD_ ), IPREPAD,
     $                                        IPOSTPAD, PADVAL )
                              IF( TPSD ) THEN
                                 CALL PZCHEKPAD( ICTXT, 'Generation',
     $                                           MP, NRHSQ,
     $                                           MEM( IPW-IPREPAD ),
     $                                           DESCW( LLD_ ), IPREPAD,
     $                                           IPOSTPAD, PADVAL )
...
*
                           END IF
*
*                          Calculate the amount of workspace for PZGELS
*
                           IF( M.GE.N ) THEN
...
*
                           CALL PZGELS( TRANS, M, N, NRHS, MEM( IPA ),
...
*                             check for memory overwrite
*
                              CALL PZCHEKPAD( ICTXT, 'PZGELS', MP,
     $                                        NQ, MEM( IPA-IPREPAD ),
     $                                        DESCA( LLD_ ), IPREPAD,
     $                                        IPOSTPAD, PADVAL )
                              CALL PZCHEKPAD( ICTXT, 'PZGELS', MNP,
     $                                        NRHSQ, MEM( IPX-IPREPAD ),
     $                                        DESCX( LLD_ ), IPREPAD,
     $                                        IPOSTPAD, PADVAL )
                              CALL PZCHEKPAD( ICTXT, 'PZGELS', LWORK,
     $                                        1, MEM( IPW-IPREPAD ),
     $                                        LWORK, IPREPAD,
     $                                        IPOSTPAD, PADVAL )
                           END IF
...
*
                           IF( CHECK ) THEN
*
*                             Am I going to call PZQRT17 ?
*
                              IF( ( M.GE.N .AND. ( .NOT.TPSD ) ) .OR.
     $                            ( M.LT.N .AND. TPSD ) ) THEN
*
*                                Call PZQRT17 first, A, X, and B remain
*                                unchanged.  Solving LS system
*
*                                Check amount of memory for PZQRT17
*
                                 IF( TPSD ) THEN
                                    WORKSIZ = NP*NRHSQ + NRHSP*MQ
                                    IPW2 = IPW + WORKSIZ
                                    WORKSIZ = WORKSIZ +
     $                                     ICEIL( DBLESZ*MAX( NQ, MAX(
     $                                         MQ, NRHSQ ) ), ZPLXSZ ) +
     $                                     IPOSTPAD
                                 ELSE
...
*
     ...
     $                                           PADVAL )
                              END IF
*
*                             Call PZQRT16, B will be destroyed.
*
                              IF( TPSD ) THEN
                                 WORKSIZ = MP + IPOSTPAD
                              ELSE
...
*
     ...
     $                                        PADVAL )
*
                              CALL PZQRT16( TRANS, M, N, NRHS,
     $                                      MEM( IPA ), 1, 1, DESCA,
     $                                      MEM( IPX ), 1, 1, DESCX,
     $                                      MEM( IPB ), 1, 1, DESCB,
     $                                      MEM( IPW ), RESULT( 1 ) )
*
                              CALL PZCHEKPAD( ICTXT, 'PZQRT16',
     $                                        MP, NQ,
     $                                        MEM( IPA-IPREPAD ),
     $                                        DESCA( LLD_ ),
     $                                        IPREPAD, IPOSTPAD,
...
*
                           IF( MYROW.EQ.0 .AND. MYCOL.EQ.0 ) THEN
                              ...
                              IF( M.GE.N ) THEN
*
*                                NOPS = DOPLA( 'ZGEQRF', M, N, 0, 0,
*                                NB ) + DOPLA( 'ZUNMQR', M, NRHS, N,
*                                0, NB )
*
