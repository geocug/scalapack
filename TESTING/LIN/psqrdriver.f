...
*  =======
*
*  PSQRDRIVER is the main test program for the REAL
...
*     .. External Functions ..
      ...
      REAL               PSLANGE
      EXTERNAL           ICEIL, LSAMEN, NUMROC, PSLANGE
*     ..
*     .. Intrinsic Functions ..
...
*                       checking routines PSLAFCHK, PSGEQRRV and
*                       PSLANGE
*
...
*
*                       Figure the amount of workspace required by the
*                       checking routines PSLAFCHK, PSGEQLRV and
*                       PSLANGE
*
                        WORKSIZ = LWORK + MP*DESCA( NB_ ) + IPOSTPAD
*
...
*
*                       Figure the amount of workspace required by the
*                       checking routines PSLAFCHK, PSGELQRV and
*                       PSLANGE
*
                        WORKSIZ = LWORK +
...
*
*                       Figure the amount of workspace required by the
*                       checking routines PSLAFCHK, PSGERQRV and
*                       PSLANGE
*
                        WORKSIZ = LWORK +
     $                            MAX( MP*DESCA( NB_ ), NQ*DESCA( MB_ )
...
*
*                       Figure the amount of workspace required by the
*                       checking routines PSLAFCHK, PSGEQRRV,
*                       PSLANGE.
*
...
*
*                       Figure the amount of workspace required by the
*                       checking routines PSLAFCHK, PSTZRZRV and
*                       PSLANGE
*
                        WORKSIZ = LWORK +
     $                            MAX( MP*DESCA( NB_ ), NQ*DESCA( MB_ )
...
*
                  CALL PSMATGEN( ICTXT, 'N', 'N', DESCA( M_ ),
     $                           DESCA( N_ ), DESCA( MB_ ),
     $                           DESCA( NB_ ), MEM( IPA ),
...
*
     ...
     $                               WORKSIZ-IPOSTPAD,
     $                               IPREPAD, IPOSTPAD, PADVAL )
                     ANORM = PSLANGE( 'I', M, N, MEM( IPA ), 1, 1,
     $                                DESCA, MEM( IPW ) )
     ...
     $                               IPOSTPAD, PADVAL )
                     CALL PSFILLPAD( ICTXT, WORKFCT-IPOSTPAD, 1,
     $                               MEM( IPW-IPREPAD ),
     $                               WORKFCT-IPOSTPAD,
     $                               IPREPAD, IPOSTPAD, PADVAL )
                  END IF
...
*  =======
*
*  PSQPPIV applies to sub( A ) = A(IA:IA+M-1,JA:JA+N-1) the pivots
*  returned by PSGEQPF in reverse order for checking purposes.
*
*  Notes
*  =====
*
...
*     .. External Subroutines ..
     ...
     $                   IGESD2D, IGAMN2D, INFOG1L, PSSWAP
*     ..
*     .. External Functions ..
...
*
         CALL PSSWAP( M, A, IA, ITMP, DESCA, 1, A, IA, J, DESCA, 1 )
*
   20 CONTINUE
*
*     End of PSQPPIV
