...
*  =======
*
*  PZGBDRIVER is a test program for the
*  ScaLAPACK Band Cholesky routines corresponding to the options
*  indicated by ZGB.  This test driver performs an
*  A = L*U factorization
...
*     .. External Subroutines ..
     ...
     $                   PZCHEKPAD, PZDBLASCHK, PZFILLPAD, PZGBINFO,
     $                   PZGBTRF, PZGBTRS, PZMATGEN, SLBOOT, SLCOMBINE,
     $                   SLTIMER
*     ..
*     .. External Functions ..
      INTEGER            NUMROC
...
*
      ...
      IBSEED = 200
*
      CALL PZGBINFO( OUTFILE, NOUT, TRANS, NMAT, NVAL, NTESTS, NBW,
...
*
     ...
     $               WRITE( NOUT, FMT = 9997 ) 'MEMORY'
               ...
               END IF
*
               CALL PZBMATGEN( ICTXT, 'G', 'N', BWL, BWU, N,
...
*
               CALL SLTIMER( 1 )
*
               CALL PZGBTRF( N, BWL, BWU, MEM( IPA ), 1, DESCA, IPIV,
...
*
               IF( INFO.NE.0 ) THEN
                  IF( IAM.EQ.0 ) THEN
                    WRITE( NOUT, FMT = * ) 'PZGBTRF INFO=', INFO
                  ENDIF
                  KFAIL = KFAIL + 1
                  GO TO 30
               END IF
...
*
                  CALL PZCHEKPAD( ICTXT, 'PZGBTRF', NP,
...
*
                     CALL PZGBTRS( TRANS, N, BWL, BWU, NRHS, MEM( IPA ),
...
*
                     IF( INFO.NE.0 ) THEN
                       IF( IAM.EQ.0 )
     $  WRITE( NOUT, FMT = * ) 'PZGBTRS INFO=', INFO
                       KFAIL = KFAIL + 1
                       PASSED = 'FAILED'
                       GO TO 20
                     END IF
...
*
                        CALL PZCHEKPAD( ICTXT, 'PZGBTRS-work',
     $                                  WORKSIZ, 1,
     $                                  MEM( IP_DRIVER_W-IPREPAD ),
     $                                  WORKSIZ, IPREPAD,
     $                                  IPOSTPAD, PADVAL )
