...
*  =======
*
*  PCPBDRIVER is a test program for the
*  ScaLAPACK Band Cholesky routines corresponding to the options
*  indicated by CPB.  This test driver performs an
...
*     .. External Subroutines ..
     ...
     $                   PCCHEKPAD, PCFILLPAD, PCMATGEN, PCPBINFO,
     $                   PCPBLASCHK, PCPBTRF, PCPBTRS, SLBOOT,
     $                   SLCOMBINE, SLTIMER
*     ..
*     .. External Functions ..
      INTEGER            NUMROC
...
*
      ...
      IBSEED = 200
*
      CALL PCPBINFO( OUTFILE, NOUT, UPLO, NMAT, NVAL, NTESTS, NBW,
...
*
     ...
     $               WRITE( NOUT, FMT = 9997 ) 'MEMORY'
               ...
               END IF
*
               CALL PCBMATGEN( ICTXT, UPLO, 'B', BW, BW, N, (BW+1), NB,
...
*
               CALL SLTIMER( 1 )
*
               CALL PCPBTRF( UPLO, N, BW, MEM( IPA ), 1, DESCA,
...
*
               IF( INFO.NE.0 ) THEN
                  IF( IAM.EQ.0 ) THEN
                    WRITE( NOUT, FMT = * ) 'PCPBTRF INFO=', INFO
                  ENDIF
                  KFAIL = KFAIL + 1
                  GO TO 30
               END IF
...
*
                  CALL PCCHEKPAD( ICTXT, 'PCPBTRF', NP,
...
*
                     CALL PCPBTRS( UPLO, N, BW, NRHS, MEM( IPA ), 1,
...
*
                     IF( INFO.NE.0 ) THEN
                       IF( IAM.EQ.0 )
     $  WRITE( NOUT, FMT = * ) 'PCPBTRS INFO=', INFO
                       KFAIL = KFAIL + 1
                       PASSED = 'FAILED'
                       GO TO 20
                     END IF
...
*
                        CALL PCCHEKPAD( ICTXT, 'PCPBTRS-work',
     $                                  WORKSIZ, 1,
     $                                  MEM( IP_DRIVER_W-IPREPAD ),
     $                                  WORKSIZ, IPREPAD,
     $                                  IPOSTPAD, PADVAL )
...
*
                        SRESID = ZERO
*
                        CALL PCPBLASCHK( 'H', UPLO, N, BW, BW, NRHS,
     $                              MEM( IPB ), 1, 1, DESCB2D,
