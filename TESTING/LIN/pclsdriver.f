...
*  =======
*
*  PCLSDRIVER is the main test program for the COMPLEX
...
*     .. External Subroutines ..
     ...
     $                   PCFILLPAD, PCGELS, PCGEMM, PCLACPY,
     $                   PCLSINFO, PCMATGEN, PSCNRM2,
     $                   PCSSCAL, PCQRT13, PCQRT16, SLBOOT,
     $                   SLCOMBINE, SLTIMER
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER            ICEIL, ILCM, NUMROC
      REAL               PCLANGE, PCQRT14, PCQRT17
      EXTERNAL           ICEIL, ILCM, LSAME, NUMROC, PCLANGE,
     $                   PCQRT14, PCQRT17
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
...
*
      IASEED = 100
      IBSEED = 200
      CALL PCLSINFO( OUTFILE, NOUT, NMAT, MVAL, NTESTS, NVAL,
...
*
                  CALL PCQRT13( ISCALE, M, N, MEM( IPA ), 1, 1,
     $                          DESCA, ANORM, IASEED, MEM( IPW ) )
*
                  IF( CHECK ) THEN
                     CALL PCCHEKPAD( ICTXT, 'PCQRT13', MP, NQ,
     $                               MEM( IPA-IPREPAD ), DESCA( LLD_ ),
     $                               IPREPAD, IPOSTPAD, PADVAL )
                     CALL PCCHEKPAD( ICTXT, 'PCQRT13',
     $                               WORKSIZ-IPOSTPAD, 1,
     $                               MEM( IPW-IPREPAD ),
     $                               WORKSIZ-IPOSTPAD, IPREPAD,
     $                               IPOSTPAD, PADVAL )
...
*
                           IF( CHECK ) THEN
                              CALL PCFILLPAD( ICTXT, MNP, NRHSQ,
     $                                        MEM( IPX-IPREPAD ),
     $                                        DESCX( LLD_ ), IPREPAD,
     $                                        IPOSTPAD, PADVAL )
                              IF( TPSD ) THEN
                                 CALL PCFILLPAD( ICTXT, MP, NRHSQ,
     $                                           MEM( IPW-IPREPAD ),
     $                                           DESCW( LLD_ ), IPREPAD,
     $                                           IPOSTPAD, PADVAL )
                              ELSE
                                 CALL PCFILLPAD( ICTXT, NP, NRHSQ,
     $                                           MEM( IPW-IPREPAD ),
     $                                           DESCW( LLD_ ), IPREPAD,
     $                                           IPOSTPAD, PADVAL )
                              END IF
...
*
                           DO 10 JJ = 1, NRHS
                              CALL PSCNRM2( NCOLS, BNORM, MEM( IPW ),
     $                                      1, JJ, DESCW, 1 )
                              IF( BNORM.GT.RZERO )
   ...
   10                      CONTINUE
*
                           CALL PCGEMM( TRANS, 'N', NROWS, NRHS, NCOLS,
...
*
                              CALL PCCHEKPAD( ICTXT, 'Generation', MP,
     $                                        NQ, MEM( IPA-IPREPAD ),
     $                                        DESCA( LLD_ ), IPREPAD,
     $                                        IPOSTPAD, PADVAL )
                              CALL PCCHEKPAD( ICTXT, 'Generation', MNP,
     $                                        NRHSQ, MEM( IPX-IPREPAD ),
     $                                        DESCX( LLD_ ), IPREPAD,
     $                                        IPOSTPAD, PADVAL )
                              IF( TPSD ) THEN
                                 CALL PCCHEKPAD( ICTXT, 'Generation',
     $                                           MP, NRHSQ,
     $                                           MEM( IPW-IPREPAD ),
     $                                           DESCW( LLD_ ), IPREPAD,
     $                                           IPOSTPAD, PADVAL )
...
*
                           END IF
*
*                          Calculate the amount of workspace for PCGELS
*
                           IF( M.GE.N ) THEN
...
*
                           CALL PCGELS( TRANS, M, N, NRHS, MEM( IPA ),
...
*                             check for memory overwrite
*
                              CALL PCCHEKPAD( ICTXT, 'PCGELS', MP,
     $                                        NQ, MEM( IPA-IPREPAD ),
     $                                        DESCA( LLD_ ), IPREPAD,
     $                                        IPOSTPAD, PADVAL )
                              CALL PCCHEKPAD( ICTXT, 'PCGELS', MNP,
     $                                        NRHSQ, MEM( IPX-IPREPAD ),
     $                                        DESCX( LLD_ ), IPREPAD,
     $                                        IPOSTPAD, PADVAL )
                              CALL PCCHEKPAD( ICTXT, 'PCGELS', LWORK,
     $                                        1, MEM( IPW-IPREPAD ),
     $                                        LWORK, IPREPAD,
     $                                        IPOSTPAD, PADVAL )
                           END IF
...
*
                           IF( CHECK ) THEN
*
*                             Am I going to call PCQRT17 ?
*
                              IF( ( M.GE.N .AND. ( .NOT.TPSD ) ) .OR.
     $                            ( M.LT.N .AND. TPSD ) ) THEN
*
*                                Call PCQRT17 first, A, X, and B remain
*                                unchanged.  Solving LS system
*
*                                Check amount of memory for PCQRT17
*
                                 IF( TPSD ) THEN
                                    WORKSIZ = NP*NRHSQ + NRHSP*MQ
                                    IPW2 = IPW + WORKSIZ
                                    WORKSIZ = WORKSIZ +
     $                                     ICEIL( REALSZ*MAX( NQ, MAX(
     $                                         MQ, NRHSQ ) ), CPLXSZ ) +
     $                                     IPOSTPAD
                                 ELSE
...
*
     ...
     $                                           PADVAL )
                              END IF
*
*                             Call PCQRT16, B will be destroyed.
*
                              IF( TPSD ) THEN
                                 WORKSIZ = MP + IPOSTPAD
                              ELSE
...
*
     ...
     $                                        PADVAL )
*
                              CALL PCQRT16( TRANS, M, N, NRHS,
     $                                      MEM( IPA ), 1, 1, DESCA,
     $                                      MEM( IPX ), 1, 1, DESCX,
     $                                      MEM( IPB ), 1, 1, DESCB,
     $                                      MEM( IPW ), RESULT( 1 ) )
*
                              CALL PCCHEKPAD( ICTXT, 'PCQRT16',
     $                                        MP, NQ,
     $                                        MEM( IPA-IPREPAD ),
     $                                        DESCA( LLD_ ),
     $                                        IPREPAD, IPOSTPAD,
...
*
                           IF( MYROW.EQ.0 .AND. MYCOL.EQ.0 ) THEN
                              ...
                              IF( M.GE.N ) THEN
*
*                                NOPS = SOPLA( 'CGEQRF', M, N, 0, 0,
*                                NB ) + SOPLA( 'CUNMQR', M, NRHS, N,
*                                0, NB )
*
