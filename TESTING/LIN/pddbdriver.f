...
*  =======
*
*  PDDBDRIVER is a test program for the
*  ScaLAPACK Band Cholesky routines corresponding to the options
*  indicated by DDB.  This test driver performs an
*  A = L*U factorization
...
*     .. External Subroutines ..
     ...
     $                   PDCHEKPAD, PDDBINFO, PDDBLASCHK, PDDBTRF,
     $                   PDDBTRS, PDFILLPAD, PDMATGEN, SLBOOT,
     $                   SLCOMBINE, SLTIMER
*     ..
*     .. External Functions ..
      INTEGER            NUMROC
...
*
      ...
      IBSEED = 200
*
      CALL PDDBINFO( OUTFILE, NOUT, TRANS, NMAT, NVAL, NTESTS, NBW,
...
*
     ...
     $               WRITE( NOUT, FMT = 9997 ) 'MEMORY'
               ...
               END IF
*
               CALL PDBMATGEN( ICTXT, 'G', 'D', BWL, BWU, N,
...
*
               CALL SLTIMER( 1 )
*
               CALL PDDBTRF( N, BWL, BWU, MEM( IPA ), 1, DESCA,
...
*
                  CALL PDCHEKPAD( ICTXT, 'PDDBTRF', NP,
...
*
                     CALL PDDBTRS( TRANS, N, BWL, BWU, NRHS, MEM( IPA ),
...
*
                     IF( INFO.NE.0 ) THEN
                       IF( IAM.EQ.0 )
     $  WRITE( NOUT, FMT = * ) 'PDDBTRS INFO=', INFO
                       KFAIL = KFAIL + 1
                       PASSED = 'FAILED'
                       GO TO 20
                     END IF
...
*
                        CALL PDCHEKPAD( ICTXT, 'PDDBTRS-work',
     $                                  WORKSIZ, 1,
     $                                  MEM( IP_DRIVER_W-IPREPAD ),
     $                                  WORKSIZ, IPREPAD,
     $                                  IPOSTPAD, PADVAL )
