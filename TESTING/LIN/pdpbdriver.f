...
*  =======
*
*  PDPBDRIVER is a test program for the
*  ScaLAPACK Band Cholesky routines corresponding to the options
*  indicated by DPB.  This test driver performs an
...
*     .. External Subroutines ..
     ...
     $                   PDCHEKPAD, PDFILLPAD, PDMATGEN, PDPBINFO,
     $                   PDPBLASCHK, PDPBTRF, PDPBTRS, SLBOOT,
     $                   SLCOMBINE, SLTIMER
*     ..
*     .. External Functions ..
      INTEGER            NUMROC
...
*
      ...
      IBSEED = 200
*
      CALL PDPBINFO( OUTFILE, NOUT, UPLO, NMAT, NVAL, NTESTS, NBW,
...
*
     ...
     $               WRITE( NOUT, FMT = 9997 ) 'MEMORY'
               ...
               END IF
*
               CALL PDBMATGEN( ICTXT, UPLO, 'B', BW, BW, N, (BW+1), NB,
...
*
               CALL SLTIMER( 1 )
*
               CALL PDPBTRF( UPLO, N, BW, MEM( IPA ), 1, DESCA,
...
*
               IF( INFO.NE.0 ) THEN
                  IF( IAM.EQ.0 ) THEN
                    WRITE( NOUT, FMT = * ) 'PDPBTRF INFO=', INFO
                  ENDIF
                  KFAIL = KFAIL + 1
                  GO TO 30
               END IF
...
*
                  CALL PDCHEKPAD( ICTXT, 'PDPBTRF', NP,
...
*
                     CALL PDPBTRS( UPLO, N, BW, NRHS, MEM( IPA ), 1,
...
*
                     IF( INFO.NE.0 ) THEN
                       IF( IAM.EQ.0 )
     $  WRITE( NOUT, FMT = * ) 'PDPBTRS INFO=', INFO
                       KFAIL = KFAIL + 1
                       PASSED = 'FAILED'
                       GO TO 20
                     END IF
...
*
                        CALL PDCHEKPAD( ICTXT, 'PDPBTRS-work',
     $                                  WORKSIZ, 1,
     $                                  MEM( IP_DRIVER_W-IPREPAD ),
     $                                  WORKSIZ, IPREPAD,
     $                                  IPOSTPAD, PADVAL )
...
*
                        SRESID = ZERO
*
                        CALL PDPBLASCHK( 'S', UPLO, N, BW, BW, NRHS,
     $                              MEM( IPB ), 1, 1, DESCB2D,
