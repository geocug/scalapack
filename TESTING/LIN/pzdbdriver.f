...
*  =======
*
*  PZDBDRIVER is a test program for the
*  ScaLAPACK Band Cholesky routines corresponding to the options
*  indicated by ZDB.  This test driver performs an
*  A = L*U factorization
...
*     .. External Subroutines ..
     ...
     $                   PZCHEKPAD, PZDBINFO, PZDBLASCHK, PZDBTRF,
     $                   PZDBTRS, PZFILLPAD, PZMATGEN, SLBOOT,
     $                   SLCOMBINE, SLTIMER
*     ..
*     .. External Functions ..
      INTEGER            NUMROC
...
*
      ...
      IBSEED = 200
*
      CALL PZDBINFO( OUTFILE, NOUT, TRANS, NMAT, NVAL, NTESTS, NBW,
...
*
     ...
     $               WRITE( NOUT, FMT = 9997 ) 'MEMORY'
               ...
               END IF
*
               CALL PZBMATGEN( ICTXT, 'G', 'D', BWL, BWU, N,
...
*
               CALL SLTIMER( 1 )
*
               CALL PZDBTRF( N, BWL, BWU, MEM( IPA ), 1, DESCA,
...
*
               IF( INFO.NE.0 ) THEN
                  IF( IAM.EQ.0 ) THEN
                    WRITE( NOUT, FMT = * ) 'PZDBTRF INFO=', INFO
                  ENDIF
                  KFAIL = KFAIL + 1
                  GO TO 30
               END IF
...
*
                  CALL PZCHEKPAD( ICTXT, 'PZDBTRF', NP,
...
*
                     CALL PZDBTRS( TRANS, N, BWL, BWU, NRHS, MEM( IPA ),
...
*
                     IF( INFO.NE.0 ) THEN
                       IF( IAM.EQ.0 )
     $  WRITE( NOUT, FMT = * ) 'PZDBTRS INFO=', INFO
                       KFAIL = KFAIL + 1
                       PASSED = 'FAILED'
                       GO TO 20
                     END IF
...
*
                        CALL PZCHEKPAD( ICTXT, 'PZDBTRS-work',
     $                                  WORKSIZ, 1,
     $                                  MEM( IP_DRIVER_W-IPREPAD ),
     $                                  WORKSIZ, IPREPAD,
     $                                  IPOSTPAD, PADVAL )
