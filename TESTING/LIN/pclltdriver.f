...
*  =======
*
*  PCLLTDRIVER is the main test program for the COMPLEX
...
*     .. External Subroutines ..
     ...
     $                   IGSUM2D, BLACS_PINFO, PCCHEKPAD, PCFILLPAD,
     $                   PCLAFCHK, PCLASCHK, PCLLTINFO,
     $                   PCMATGEN, PCPOCON, PCPORFS,
     $                   PCPOTRF, PCPOTRRV, PCPOTRS, SLBOOT,
     $                   SLCOMBINE, SLTIMER
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      ...
      REAL               PCLANHE
      EXTERNAL           ICEIL, ILCM, LSAME, NUMROC, PCLANHE
*     ..
*     .. Intrinsic Functions ..
...
*
      ...
      IASEED = 100
      IBSEED = 200
      CALL PCLLTINFO( OUTFILE, NOUT, UPLO, NMAT, NVAL, NTESTS, NNB,
...
*                 the checking routines PCLAFCHK, PCPOTRRV, and
*                 PCLANHE
*
...
*
               CALL PCMATGEN( ICTXT, 'Herm', 'Diag', DESCA( M_ ),
...
*
     ...
     $                             DESCA, MEM( IPW ) )
                  ANORM1 = PCLANHE( '1', UPLO, N, MEM( IPA ), 1, 1,
     $                             DESCA, MEM( IPW ) )
                  CALL PCCHEKPAD( ICTXT, 'PCLANHE', NP, NQ,
     $                            MEM( IPA-IPREPAD ), DESCA( LLD_ ),
     $                            IPREPAD, IPOSTPAD, PADVAL )
                  CALL PCCHEKPAD( ICTXT, 'PCLANHE', WORKSIZ-IPOSTPAD,
     $                            1, MEM( IPW-IPREPAD ),
...
*
               IF( EST ) THEN
*
*                 Calculate workspace required for PCPOCON
*
                  LWORK = MAX( 1, 2*NP ) +
     $                    MAX( 2, DESCA( NB_ )*
...
*
*                       Calculate the amount of workspace required by
*                       the checking routines PCLASCHK
*
                        LCMQ = LCM / NPCOL
...
*
                        CALL PCCHEKPAD( ICTXT, 'PCPOTRS', NP, NQ,
     $                                  MEM( IPA-IPREPAD ),
     $                                  DESCA( LLD_ ),
     $                                  IPREPAD, IPOSTPAD, PADVAL )
                        CALL PCCHEKPAD( ICTXT, 'PCPOTRS', NP,
...
*
                        CALL PCLASCHK( 'Herm', 'Diag', N, NRHS,
     $                                 MEM( IPB ), 1, 1, DESCB,
...
*
                     IF( EST ) THEN
*
*                       Calculate workspace required for PCPORFS
*
                           LWORK = MAX( 1, 2*NP )
                           IPW2  = IPW + LWORK + IPOSTPAD + IPREPAD
                           LRWORK = MAX( 1, NP )
...
*
     ...
     $                                        LWORK, IPREPAD, IPOSTPAD,
     $                                        PADVAL )
                              CALL PCFILLPAD( ICTXT, LW2-IPOSTPAD,
     $                                        1, MEM( IPW2-IPREPAD ),
     $                                        LW2-IPOSTPAD,
     $                                        IPREPAD, IPOSTPAD,
     $                                        PADVAL )
...
*
     ...
     $                                        DESCB( LLD_ ), IPREPAD,
     $                                        IPOSTPAD, PADVAL )
                              CALL PCCHEKPAD( ICTXT, 'PCPORFS', 1,
     $                                        MYRHS,
     $                                        MEM( IPFERR-IPREPAD ), 1,
     $                                        IPREPAD, IPOSTPAD,
     $                                        PADVAL )
     ...
     $                                        IPREPAD, IPOSTPAD,
     $                                        PADVAL )
                              CALL PCCHEKPAD( ICTXT, 'PCPORFS', LWORK,
     $                                        1, MEM( IPW-IPREPAD ),
     $                                        LWORK, IPREPAD, IPOSTPAD,
     $                                        PADVAL )
                              CALL PCCHEKPAD( ICTXT, 'PCPORFS',
     $                                        LW2-IPOSTPAD, 1,
     $                                        MEM( IPW2-IPREPAD ),
     $                                        LW2-IPOSTPAD,
     $                                        IPREPAD, IPOSTPAD,
...
*
                  CALL PCPOTRRV( UPLO, N, MEM( IPA ), 1, 1, DESCA,
     $                           MEM( IPW ) )
                  CALL PCLAFCHK( 'Symm', 'Diag', N, N, MEM( IPA ), 1, 1,
     $                           DESCA, IASEED, ANORM, FRESID,
     $                           MEM( IPW ) )
*
*                 Check for memory overwrite
*
                  CALL PCCHEKPAD( ICTXT, 'PCPOTRRV', NP, NQ,
     $                            MEM( IPA-IPREPAD ), DESCA( LLD_ ),
     $                            IPREPAD, IPOSTPAD, PADVAL )
